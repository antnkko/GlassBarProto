...
## 13. Етап 6: справжній variableBlur — 1:1 з системою

Фінальний акорд по краях. Рісьорч (дизасембл UIKitCore/CoreMaterial з iOS 26.3 рантайму + ком'юніті-реверси) показав, що системний soft edge — це не «блюр з маскою прозорості», а приватний `CAFilter("variableBlur")` на бекдроп-шарі: вздовж смуги міняється РАДІУС розмиття, контент лишається непрозорим. Витягнуті числа: маска = gaussian falloff 24pt від межі бара, хром-матеріал блюрить на 22.5pt, затемнення поверх — символічні 0.01.

Тому в GlassEdge з'явився режим «Apple» (дефолт): `VariableBlurUIView` — UIVisualEffectView, якому в бекдроп-шар підкладається той самий приватний фільтр (radius 22.5, normalized edges), а тінт-хром ефект-в'юхи обнуляється (у системного ефекту його і нема). Маска-рампа генерується з тих самих ручок панелі — fadeStart/curve/intensity — тільки тепер вони керують радіусом, а не прозорістю: альфа маски ∝ локальний радіус блюру. Дефолти з рісьорчу: лінійна рампа (curve 1 — верифікований ком'юніті матч), fadeStart 0.3, смуги по 100pt.

Чотири Material-варіанти лишились у сегментах як легальний фолбек — для Numo-проду або приватний API через обгортку типу aheze/VariableBlurView (шипається в App Store), або Material-режим. Всі приватні виклики за guard-ами: якщо CAFilter зникне в якомусь iOS, в'юха тихо деградує у звичайний блюр.

Приємний сюрприз: variableBlur рендериться на симуляторі (це рівень Core Animation, не той приватний edge-пайплайн) — краї тепер повністю ітеруються скріншотами.
